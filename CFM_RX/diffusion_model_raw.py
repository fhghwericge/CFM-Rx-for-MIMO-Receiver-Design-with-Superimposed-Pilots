...
class DiffusionModel(nn.Module):
    ...
    def forward(self, x_0: torch.Tensor, noise: torch.Tensor = None, t: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.data_start_t > -1:
            ...
            t_start = None
        x_t = self.forward_sample(x_0, t, t_start=t_start, noise=noise)
        ...
        if self.objective == 'pred_post_mean':
            ...
            loss = self.loss_fn(model_out, x, reduction=reduction)
        elif self.objective == 'pred_noise':
            loss = self.loss_fn(model_out, noise, reduction=reduction)
            loss = [loss,0]
        elif self.objective == 'pred_x_0':

            ssim_value = ssim(torch.nn.functional.sigmoid(model_out), torch.nn.functional.sigmoid(x_0),data_range=1.0)
    ...
    def train(self) -> Dict:
        ...
        while self.epoch <= self.num_epochs:
            ...
            with torch.no_grad():
                ...
                mmds.append(mmd)
                self.checkpoint += 1
                print(f'Epoch {self.epoch}/{self.num_epochs + 1}: train_loss = {train_losses[-1] :.8f}')
