...
class DiffusionModel(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.data_start_t = data_start_t
        ...
        alphas_cumprod = torch.square(torch.linspace(0.999, 0.001, steps=100,device=self.device))
        snrs = alphas_cumprod / (1. - alphas_cumprod)
        alphas = torch.zeros(100,device=self.device)
    ...
    def forward_sample(self, x_0: torch.Tensor, t: torch.Tensor, t_start: torch.Tensor = None, noise: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        # t_start = utils.default(t_start, lambda: torch.zeros_like(t,dtype=torch.int64))
        ...
        if t_start is None:
            ...
            assert t_start.shape == t.shape
            return (
                (utils.extract(self.sqrt_alphas_cumprod, t, x_0.shape) / utils.extract(self.sqrt_alphas_cumprod, t_start, x_0.shape)) * x_0 +
                torch.sqrt(torch.ones_like(x_0)-((utils.extract(self.sqrt_alphas_cumprod, t, x_0.shape) / utils.extract(self.sqrt_alphas_cumprod, t_start, x_0.shape)))) * noise
            )
    ...
    def reverse_step(self, x_t: torch.Tensor, t: int, *, add_random: bool = False) -> torch.Tensor:
        ...
        if self.reverse_method == 'reverse_mean':
            ...
            if add_random:
                ...
                # print(f't: {t}, posterior_mean.var: {posterior_mean.var()}, noise.var: {(torch.sqrt(posterior_variance) * noise).var()}')
            else:
                # Reverse Mean Forwarding process
    ...
    def reverse_sample_loop(self, x_t: torch.Tensor, t_start: int,
                            *, return_all_timesteps: bool = False, add_random: bool = False, t_end = 0) -> torch.Tensor:
        """
    ...
    def generate_new_samples(self, n_samples: int, *, noise: torch.Tensor = None, return_all_timesteps: bool = False,
                             add_random: bool = None, t_end = 0) -> torch.Tensor:
        """
        ...
        x_0 = self.reverse_sample_loop(x_t, self.num_timesteps, return_all_timesteps=return_all_timesteps,
                                       add_random=add_random, t_end=t_end)
        return x_0

    @torch.no_grad()
    def generate_estimate(self, y: torch.Tensor, snr: float, *, add_random: bool = None,
                          return_all_timesteps: bool = False, t_end=0, snr_dB = False) -> torch.Tensor:
        """
        ...
        if t < t_end:
            ...
            x_hat = self.reverse_sample_loop(x_t, t, return_all_timesteps=return_all_timesteps, add_random=add_random,t_end=t_end)

        return x_hat
    @torch.no_grad()
    def generate_estimate_2(self, y: torch.Tensor, snr: float, *, add_random: bool = None,
                          return_all_timesteps: bool = False, t_end=0, snr_dB = False, A: torch.Tensor, X: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x_0: torch.Tensor, noise: torch.Tensor = None, t: torch.Tensor = None) -> torch.Tensor:
        ...
        # sample_size_list = [int(b*0.3),int(b*0.6),b - int(b*0.3)- int(b*0.6)]
        # t_list = [7,23,68] #SNRdB=[15,5,-10]
        # # sample_size_list = [int(b/3), b - int(b/3)]
        # # t_list = [7,23] #SNRdB=[15,5,-10]
        # t_pre = torch.cat([torch.full((size,), t, device=self.device) for size, t in zip(sample_size_list, t_list)])
        # x_pre = self.forward_sample(x_0, t=t_pre, t_start=None)
        # if self.method == 'Schedule_A':
        #     t_list = [14,40,80]
        #     t_target = torch.cat([torch.randint(t, 100, (size,), device=self.device).long() for size, t in zip(sample_size_list, t_list)])
        #     noise = utils.default(noise, lambda: self.noise_multiplier * torch.randn_like(x_0))
        #     x_t = self.forward_sample(x_pre, t=t_target, t_start=t_pre, noise=noise)
        #     model_out = self.model(x_t, t_target)
        # elif self.method == 'Schedule_B':
        #     t_list=[7,23,68,100]
        #     t_target = torch.cat([torch.randint(t_list[i], t_list[i+1], (sample_size_list[i],), device=self.device).long() for i in range(len(sample_size_list))])
        #     noise = utils.default(noise, lambda: self.noise_multiplier * torch.randn_like(x_0))
        #     x_t = self.forward_sample(x_pre, t=t_target, t_start=t_pre, noise=noise)
        #     model_out = self.model(x_t, t_target)
        
        # elif self.method == 'Schedule_C':
        #     t_target = []

        #     density_list = [(1e8, 100, 100)]
        #     for t, s in zip(reversed(t_list), reversed(sample_size_list)):
        #         t_d_l = density_list[:]
        #         remaining_size = s
        #         d_0 = 0
        #         for d,t_s,t_e in density_list:
        #             d_c = remaining_size / (t_s - t) + d_0
        #             if d_c <= d:
        #                 t_d_l = [(d_c, t, t_s)] + t_d_l
        #                 t_target.append(torch.randint(t, t_s, (remaining_size,))) 
        #                 break
        #             else:
        #                 s_0 = int((t_s - t)*d)
        #                 t_target.append(torch.randint(t, t_s, (s_0,)))
        #                 d_0 = d
        #                 t_d_l = t_d_l[1:]
        #                 remaining_size = remaining_size - s_0
        #         density_list = t_d_l[:]

        #     t_target = torch.cat(t_target[::-1])
        #     t_target = t_target.to(device=self.device)
        #     noise = utils.default(noise, lambda: self.noise_multiplier * torch.randn_like(x_0))
        #     x_t = self.forward_sample(x_pre, t=t_target, t_start=t_pre, noise=noise)
        #     model_out = self.model(x_t, t_target)
        if self.method == 'No_schedule':

            # DM forward process
            # t = utils.default(t, lambda: torch.randint(self.data_start_t+1, self.num_timesteps, (b,), device=self.device).long())
            t = torch.rand(b,device=self.device)
            noise = utils.default(noise, lambda: self.noise_multiplier * torch.randn_like(x_0))
            if self.data_start_t > -1:
                t_start = torch.ones_like(t,dtype=torch.int64) * self.data_start_t
            else:
                t_start = None
            x_t = self.forward_sample(x_0, t=t, t_start=t_start, noise=noise)
        ...
        if self.objective == 'pred_post_mean':
            ...
            loss = self.loss_fn(model_out, x_0 - noise, reduction=reduction)
            loss = [loss,0]
        elif self.objective == 'pred_x_0':

            ssim_value = ssim(torch.nn.functional.relu(model_out), torch.nn.functional.relu(x_0),data_range=1.0)
            ssim_loss = (1 - ssim_value)/50
            mse_loss = self.loss_fn(model_out, x_0, reduction=reduction)
...
class Trainer(object):
    def __init__(self,
        ...
        self.dataloader = DataLoader(data_train, batch_size=self.batch_size, shuffle=False, pin_memory=False)
        ...
        if track_val_loss:
            ...
            self.t_val = torch.randint(self.model.data_start_t+1, self.model.num_timesteps, (self.num_val_samples,)).long()
            ...
            if not old_files:
                ...
                try:
                    ...
                    if save_new:
                        ...
                        # print('model saved, epoch=',self.epoch)
    ...
    def validate(self, loss) -> Tuple[float, float, float]:
        """
        ...
        with torch.no_grad():
            ...
            # print('mse_loss:',val_loss[0])    
            # print('ssim_loss:',val_loss[1])
            if self.use_ssim:
    ...
    def train(self) -> Dict:
        ...
        while self.epoch <= self.num_epochs:
            ...
            val_loss, fid_score, mmd = self.validate(loss)
            val_losses.append(val_loss)
            fid_scores.append(fid_score) if fid_score is not None else None
            mmds.append(mmd) if mmd is not None else None
            self.checkpoint += 1

            print(f'Epoch {self.epoch}/{self.num_epochs + 1}: train_loss = {train_losses[-1]:.8f}, val_loss = {val_loss:.8f}')
            
            if early_stopping(val_loss=val_loss, epoch=self.epoch):
...
class Tester(object):
    def __init__(self,
        ...
        self.dataloader = DataLoader(self.data, batch_size=batch_size, shuffle=False, pin_memory=False)
    ...
    def __call__(self, val_loss, epoch):
        if epoch > self.min_epochs:
            if val_loss < self.best_val_loss:
                ...
                self.counter -= 1
                print(f'EarlyStopping counter: {self.num_epochs_no_improve-self.counter} out of {self.num_epochs_no_improve}')
        if self.counter < 1:
            return True
        else:
            return False
